...
router.get('/:timestamp', (req, res) => {
  let keyTimeStamp =  new Date(req.params.timestamp);
  if (isNaN(keyTimeStamp)) throw new HttpError(400);
  
  const result = store.fetch(keyTimeStamp);

  if (result) res.status(200).json(serializeMeasurement(result));
